...
function bizz_image($args) {
	...
	*/
	...
	elseif ( isset($GLOBALS['opt']['bizzthemes_thumb_show']) && ! empty( $thumb_id ) ) {
		if ( isset($GLOBALS['opt']['bizzthemes_auto_img']) ) {
			if ( 0 == $height ) {
				$img_data = wp_get_attachment_image_src( $thumb_id, array( intval( $width ), 9999 ) );
				$height = $img_data[2];
			}
			// Dynamically resize the post thumbnail
			$vt_crop = isset( $GLOBALS['opt']['bizzthemes_resize'] ) ? true : false;
			if ( $vt_crop ) $vt_crop = true; else $vt_crop = false;
			...
			$img_link = get_the_post_thumbnail( $id, $thumb_size, array( 'class' => 'bizz-thumb ' . esc_attr( $class ) ) );
		}
	}
	...
	if ( empty( $custom_field ) && isset($GLOBALS['opt']['bizzthemes_auto_img']) && empty( $img_link ) && ! ( is_singular() && in_the_loop() && $link == 'src' ) ) {
		...
		if ( ! empty( $attachments ) ) {
			...
			foreach ( $attachments as $att_id => $attachment ) {
				...
				if ( isset($GLOBALS['opt']['bizzthemes_thumb_show']) && isset($GLOBALS['opt']['bizzthemes_auto_img']) ) {
					// Dynamically resize the post thumbnail
					$vt_crop = isset( $GLOBALS['opt']['bizzthemes_resize'] ) ? true : false;
	...
	// Set standard class
	if ( $class ) $class = 'bizz-thumb ' . esc_attr( $class ); else $class = 'bizz-thumb';
...
function bizz_activation_alert() {
    if ( ($GLOBALS['opt']['themeid'] != $GLOBALS['themeid']) || (empty($GLOBALS['opt']) ) ) {
...
function bizz_headline() {
	...
	if (is_404()) {
		...
		if ( is_front_page() || is_page_template('template-blog.php') )
...
function hosted_twitter_script( $twitid = '', $username = '', $count=1 ) {
	...
	if ( empty($tweet_saved) ) {
		$twitter_json_url = esc_url( "https://api.twitter.com/1.1/statuses/user_timeline.json?include_rts=true&screen_name=$account", null, 'raw' );
...
function _widget_twitter_username( $matches ) { // $matches has already been through esc_html
	return "$matches[1]@<a href='" . esc_url( 'https://twitter.com/' . urlencode( $matches[2] ) ) . "'>$matches[2]</a>";
}

function _widget_twitter_hashtag( $matches ) { // $matches has already been through esc_html
	return "$matches[1]<a href='" . esc_url( 'https://search.twitter.com/search?q=%23' . urlencode( $matches[2] ) ) . "'>#$matches[2]</a>";
}
